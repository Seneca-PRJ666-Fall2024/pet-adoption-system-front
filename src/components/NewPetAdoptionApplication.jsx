...
const NewPetAdoptionApplication = ({
  newApplication,
  handleInputChange,
  attributeGroups,
}) => {
  // State to store the matched pet recommendations
  const [matchedRecommendations, setMatchedRecommendations] = useState([]);
  // State to store the selected pet recommendation
  const [selectedRecommendation, setSelectedRecommendation] = useState(null);

  // Accessing user role and token from AuthContext
  ...
  // Initialize the backend API when the token is available
  useEffect(() => {
    if (token) {
  ...
  // Fetch matched pet recommendations from the backend
  useEffect(() => {
    if (!backendApi) {
    ...
    try {
      // Fetching matching recommendations
      backendApi.matching.matchingRecommendationAcceptedGet(
        (error, data, response) => {
          if (error) {
  ...
  const handlePetSeection = (e) => {
    ...
    // Find the selected recommendation by ID
    if (value) {
      const selectedRec = Array.isArray(matchedRecommendations)
        ? matchedRecommendations.find((r) => r.id === value)
        : null;
      if (selectedRec) {
    ...
    <Form>
      ...
      {selectedRecommendation && (
        <PetProfile
          ...
          imageSrc={
            selectedRecommendation.pet.imageUrl
              ? backendApi.imagePath(selectedRecommendation.pet.imageUrl)
              : ""
          }
        />
      )}
      ...
      <Form.Group controlId="formHoursAlone" className="mt-2">
        <Form.Label style={{ fontWeight: "bold" }}>
          How many hours will the pet be left alone daily?
        </Form.Label>
        <Form.Control
          type="number"
          name="hoursAlone"
      ...
      <Form.Group controlId="formCrateTraining" className="mt-2">
        <Form.Label style={{ fontWeight: "bold" }}>
          Do you plan to use crate training?
        </Form.Label>
        <Form.Control
          type="text"
          name="crateTraining"
      ...
      <Form.Group controlId="formPriorExperience" className="mt-2">
        <Form.Label style={{ fontWeight: "bold" }}>
          Do you have prior experience with pets?
        </Form.Label>
        <Form.Control
          type="text"
      ...
      <Form.Group controlId="formCurrentPets" className="mt-2">
        <Form.Label style={{ fontWeight: "bold" }}>
          Do you have other pets? Please provide details.
        </Form.Label>
        <Form.Control
          type="text"
          name="currentPets"
      ...
      <Form.Group controlId="formGroomingPlan" className="mt-2">
        <Form.Label style={{ fontWeight: "bold" }}>
          How often will you groom your pet?
        </Form.Label>
        <Form.Control
          type="text"
          name="groomingPlan"
      ...
      <Form.Group controlId="formBehavioralExpectations" className="mt-2">
        <Form.Label style={{ fontWeight: "bold" }}>
          What are your behavioral expectations for your pet?
        </Form.Label>
        <Form.Control
          type="text"
